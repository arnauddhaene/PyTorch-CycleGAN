...
# Dataset loader
transforms_ = [
    transforms.Resize(int(opt.size * 1.12), interpolation=transforms.InterpolationMode.BICUBIC),
...
dataset = ImageDataset(opt.dataroot, transforms_=transforms_, unaligned=True, grayscale=(opt.input_nc == 1))
dataloader = DataLoader(dataset, batch_size=opt.batchSize, shuffle=True, num_workers=opt.n_cpu)

# Create logger for loss plots in Visdom
